...
def register_admin(name, last_name, password):
    if is_valid_name(name) and is_valid_name(last_name) and is_valid_password(password):
        ...
        email = generate_email(name, last_name, "admin")
        db_writer.register_user(username, password, email, "admin")
...
def register_parent(name, last_name, phone_number, email, address):
    if is_valid_name(name) and is_valid_name(last_name):
...
def register_staff(name, last_name, password, branch, role):
    if is_valid_name(name) and is_valid_name(last_name) and is_valid_password(password):
        ...
        username = generate_username(name, last_name)
        email = generate_email(name, last_name, role)
        staff_id = db_writer.register_user(username, password, role)
        if staff_id:
...
def register_class(name, hour, room, teacher_id):
    if is_valid_name(name):
        db_writer.register_class(name, hour, room, teacher_id)
...
def generate_email(name, last_name, role):
    clean_name = name.split()[0].lower()
    clean_last_name = last_name.lower().replace(" ", "")
    return f"{clean_name}.{clean_last_name}@{role}.college.edu"

def generate_username(name, last_name):
    clean_name = name.split()[0].lower()
    clean_last_name = last_name.split()[0].lower()
    return f"{clean_name}{clean_last_name}"
